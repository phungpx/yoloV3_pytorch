...
class YOLOLoss(loss.LossBase):
    def __init__(
        ...
        super(YOLOLoss, self).__init__(output_transform)
    ...
    def forward(self, preds: Tuple[torch.Tensor], targets: List[Dict[str, torch.Tensor]]) -> torch.Tensor:
        '''
        Args:
            preds: Tuple(Tensor)
        ...
        return losses[13] + losses[26] + losses[52]
