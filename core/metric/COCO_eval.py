...
class COCOEvaluator(MetricBase):
    def __init__(
        self,
        image_size: int = None,
    ...
    def update(self, output):
        ...
        for pred, target, image_info in zip(preds, targets, image_infos):
            ...
            scale = max(image_info[1]) / self.image_size if self.image_size is not None else 1.  # deal with input sample is paded to square (bottom-right)
